...
# a list more likely to give appropriate letter frequencies as seen in English
boggle_letters = 'AEANEGWNGEEHAHSPCOLNHNRASPFFKTSTIYDOBJOABOWTOATIOTMUCERTTYLRYDELTOESSILREIXDTERWHVEIUNESNUIHMQ'

# for testing the code
boggle_test_board = [['A', 'H', 'E', 'U', 'M'], ['T', 'H', 'G', 'H', 'U'], 
                     ['I', 'N', 'D', 'U', 'C'], ['X', 'R', 'I', 'A', 'N'], 
                     ['F', 'A', 'L', 'E', 'G']]
...
class Boggle():
    ...
    def find_from(self, board, word, y, x, seen):
        ...
        # special handling for letter 'q'
        if word[0] == 'Q':
            if len(word) <= 2:      # 'q' must always be followed by 'u' in English
                return False
            if word[1] != 'U':
                return False
            rest = word[2:]      # skip over 'qu' at beginning and continue with rest of word
        else:
            rest = word[1:]      # default logic, 'eat' this letter & continue

        # check up, down, left, right

        if y > 0:
            if self.find_from(board, rest, y - 1, x, seen):
                return True
        ...
        if x > 0:
            if self.find_from(board, rest, y, x - 1, seen):
                return True
        ...
        # diagonals
        if y > 0 and x > 0:
            if self.find_from(board, rest, y - 1, x - 1, seen):
                return True
